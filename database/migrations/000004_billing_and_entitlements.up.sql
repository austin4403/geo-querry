...
-- -----------------------------------------------------------------------------
-- Plan Catalog (Authoritative Server-Side Pricing)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Payment Attempts (State Machine)
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS payment_attempts (
    ...
    provider_attempt_id  VARCHAR(255), -- Safaricom CheckoutRequestID or Stripe PaymentIntent ID
    customer_identifier  VARCHAR(255), -- Sanitized Phone (254...) or Stripe Customer ID
    failure_reason       TEXT,
    created_at           TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
-- -----------------------------------------------------------------------------
-- Immutable Financial Transaction Ledger (Append-Only)
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS payment_transactions (
    ...
    provider_transaction_id VARCHAR(255) NOT NULL UNIQUE, -- Safaricom Receipt Number (e.g. QKH7...) or Stripe Charge ID
    amount_minor_units      BIGINT NOT NULL,
    currency                VARCHAR(10) NOT NULL,
...
-- Enforce append-only invariant on financial transactions
CREATE OR REPLACE FUNCTION trg_prevent_transaction_mutation()
RETURNS TRIGGER AS $$
BEGIN
    RAISE EXCEPTION 'Financial ledger transactions are immutable. UPDATE and DELETE are strictly prohibited.'
        USING ERRCODE = '55000';
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER trg_transactions_immutable
    ...
    FOR EACH ROW
    EXECUTE FUNCTION trg_prevent_transaction_mutation();

-- -----------------------------------------------------------------------------
-- Durable Webhook Event Inbox (Idempotent Delivery)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Organization Subscriptions & Entitlements
-- -----------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS organization_entitlements (
    organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    feature_key     VARCHAR(100) NOT NULL, -- e.g. 'export_shapefile', 'gis_las_processing', 'team_telemetry'
